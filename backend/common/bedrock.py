...
def _create_body(model: str, prompt: str):
    if model == "titan":
        ...
        prompt = f"""take on the persona of a doctor named 'Dr Steve'. Interview the patient one question at a time. Once you have enough information make a diagnosis and offer to find a medical facility near the patient. <text> {query} </text>"""
        parameter = {}
